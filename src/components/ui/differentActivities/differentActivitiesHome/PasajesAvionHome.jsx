...
const PasajesAvionHome = () => {
  const [pasajesAvion, setPasajesAvion] = useState([]);

  useEffect(() => {
    const fetchPasajesAvion = async () => {
      try {
        // Call the function to filter services by type
        const pasajesAvionFiltrados = await filtrarServiciosPorTipo('PASAJES_DE_AVION');
        setPasajesAvion(pasajesAvionFiltrados || []); // Set pasajesAvion or an empty array if null
      } catch (error) {
        console.error('Error fetching pasajes de avión:', error);
        setPasajesAvion([]); // Set pasajesAvion as an empty array in case of error
      }
    };

    fetchPasajesAvion(); // Fetch pasajesAvion on component load
  }, []);
    ...
    <div className="bg-gradient-to-br from-blue-400 to-purple-500 min-h-screen flex items-center justify-center overflow-x-auto">
      <div className="max-w-5xl w-full p-8 bg-white rounded-lg shadow-md border-4 border-purple-600">
        <h2 className="text-4xl font-semibold mb-8 text-center text-purple-800 py-4 rounded-md">
          <FaPlane className="inline-block mr-2" /> Pasajes de Avión Disponibles
        </h2>
        <div className="flex flex-nowrap overflow-x-auto pl-6" style={{ scrollbarWidth: 'thin', scrollbarColor: 'purple #ccc' }}>
          {Array.isArray(pasajesAvion) && pasajesAvion.length > 0 ? (
            pasajesAvion.map((pasaje, index) => (
              <div key={index} className="flex-none mr-6" style={{ scrollSnapAlign: 'start' }}>
                <ActividadesCard servicio={pasaje} />
              </div>
            ))
          ) : (
            <p className="text-gray-500 text-center col-span-3">No hay pasajes de avión disponibles.</p>
          )}
        </div>
      </div>
    </div>
...
export default PasajesAvionHome;
